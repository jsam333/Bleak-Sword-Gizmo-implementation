  ...
  <script>
    ...
    class Player {
      constructor() {
        ...
        this.width = 8.4;    // 14 * 0.6 (40% smaller total)
        this.height = 36;    // 60 * 0.6
        this.baseAngle = 0;
        this.spinAngle = 0;
      ...
      draw(ctx, chargeProgress, chargeVisible) {
        ...
        if (this.action && this.action.type === "charged") {
          const ringScale = 1.1;
          const ringLineWidth = 5 * ringScale;  // thinner ring
        ...
        if (chargeVisible && !this.action) {
          const ringScale = 1.1;
          const chargeBarLineWidth = 3;
          const chargeBarOuterRadius = this.height * 0.5 * ringScale;  // same as attack circle
    ...
    class TriangleEnemy {
      ...
      draw(ctx, nowSeconds) {
        ...
        const shadowY = this.y + this.height * 0.35 - 3.5;
        ctx.save();
        ctx.fillStyle = "rgba(0, 0, 0, 0.14)";
        ctx.beginPath();
        ctx.ellipse(this.x, shadowY, this.width * 0.5, 2.2, 0, 0, TAU);
        ctx.fill();
        ctx.fillStyle = "rgba(0, 0, 0, 0.07)";
        ctx.beginPath();
        ctx.ellipse(this.x, shadowY + 0.2, this.width * 0.7, 3.2, 0, 0, TAU);
        ctx.fill();
        ctx.restore();
        ...
        ctx.translate(this.x, this.y);
        ctx.rotate(waddle + this.rollAngle);
        ctx.font = `${ratFontSize}px sans-serif`;
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText(ratEmoji, 0, 0);
    ...
    class Game {
      constructor(canvas) {
        ...
        this.enemy = new TriangleEnemy(GAME_WIDTH * 0.72, GAME_HEIGHT * 0.28);
      ...
      update(dt) {
        ...
        this.enemy.onPlayerContact(this.player, this);
      }
      ...
      render() {
        ...
        this.enemy.draw(ctx, nowSeconds);
        ...
        ctx.font = "12px Arial";
        ctx.textAlign = "center";
        ctx.fillText("Swipe: short 180 dodge roll", GAME_WIDTH * 0.5, GAME_HEIGHT - 34);
        ctx.fillText("Hold 0.1s + fill (0.2s), then swipe: 720 charged roll", GAME_WIDTH * 0.5, GAME_HEIGHT - 16);
